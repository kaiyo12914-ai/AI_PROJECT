...
```powershell
(VENV3.12) PS H:\AI\AI_TOOLS> python --version
Python 3.12.10
```
...
```powershell
py -3.12 -m pip download pgvector -d H:\AI\WHL
```

在你現在這個 venv 裡，可以改成：

```powershell
python -m pip download pgvector -d H:\AI\WHL
```
...
```powershell
python -m pip download "psycopg[binary,pool]" pgvector sqlalchemy -d H:\AI\WHL
```

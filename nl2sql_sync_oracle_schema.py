...
This tool extracts Oracle DDL, table comments, and column comments for tables
whose names match the configured prefixes, then writes the result into:
...
def _build_table_filter(prefixes: list[str]) -> tuple[str, dict[str, Any]]:
    clauses: list[str] = []
    params: dict[str, Any] = {}
    for idx, prefix in enumerate(prefixes):
        param_name = f"table_like_{idx}"
        clauses.append(f"table_name LIKE :{param_name} ESCAPE '\\'")
        params[param_name] = prefix.replace("_", r"\_") + "%"
    return "(" + " OR ".join(clauses) + ")", params


def _build_comment_sql(owner: str, table_name: str, table_comment: str, columns: list[dict[str, Any]]) -> str:
    lines: list[str] = []
...
def _get_oracle_table_rows(oracle_conn, owner: str, prefixes: list[str]) -> list[dict[str, str]]:
    owner = _normalize_text(owner).upper()
    table_filter, params = _build_table_filter(prefixes)
    params["owner"] = owner or None
    fetch_sql = FETCH_TABLES_SQL.format(table_filter=table_filter)
...
def _extract_oracle_schema(oracle_conn, owner: str, table_name: str) -> dict[str, Any]:
    ...
    with oracle_conn.cursor() as cur:
        ...
        cur.execute(GET_ROW_ESTIMATE_SQL, params)
    ...
    comment_sql_text = _build_comment_sql(owner, table_name, table_comment, columns)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sync Oracle CT_*/DT_* tables into nl2sql_schema_object and nl2sql_schema_embedding.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        tables = _get_oracle_table_rows(oracle_conn, oracle_owner, prefixes)
        if args.limit and args.limit > 0:
            tables = tables[: args.limit]

        if args.dry_run:
            for idx, item in enumerate(tables, start=1):
                schema = _extract_oracle_schema(oracle_conn, item["owner"], item["table_name"])
                print(f"[{idx}] {schema['table_schema']}.{schema['table_name']}")
                print(schema["ddl_text"])
                print("-" * 80)
            print(f"Done. processed={len(tables)}")
            return 0

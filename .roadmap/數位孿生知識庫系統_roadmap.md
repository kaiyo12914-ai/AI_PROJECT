...
* **背景問題**：工業 4.0 與數位孿生系統中，包含大量的製造業標準（如 ISA-95）、PLC 控制協議、SCADA 數據流與故障維護指南。這些文件極具專業性，且散落於各處，傳統關鍵字檢索無法進行語意關聯問答。
* **目標使用者**：工業雙生系統開發者、現場工藝工程師、系統維運人員。
* **核心需求**：在 `H:\AI\AI_TOOLS` Django 專案中，以 PostgreSQL + `pgvector` 為核心，打造一個**內網實體隔離環境可用**、具備**安全防護控制**與 **ISA-95 層級分類**的數位孿生 RAG 智能問答與 Ingestion 知識庫 Dashboard 平台。
...
* **開發規劃**：
  ...
  - 修改 Portal `index.html` 將「數位孿生知識庫」卡片按鈕正式掛載上線。
* **驗收條件**：
...
為提升數位孿生知識庫的實戰性能與使用者體驗，特規劃以下四項精進功能：

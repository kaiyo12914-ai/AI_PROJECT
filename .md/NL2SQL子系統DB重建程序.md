...
於 `H:\AI\AI_TOOLS` 專案根目錄執行：

```powershell
H:\AI\AI_TOOLS\venv\Scripts\python.exe manage.py migrate vanna_integration
```
...
- Oracle / Sybase / 其他業務 DB 連線設定以 `H:\AI\AI_TOOLS\.env_DB_factory` 為準。
...
```powershell
H:\AI\AI_TOOLS\venv\Scripts\python.exe manage.py showmigrations vanna_integration
```
...
```powershell
H:\AI\AI_TOOLS\venv\Scripts\python.exe -m pip install --no-index --find-links H:\AI\whl -r H:\AI\AI_TOOLS\requirement_vanna.txt
```

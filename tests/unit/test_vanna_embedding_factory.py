...
class VannaEmbeddingFactoryTestCase(SimpleTestCase):
    @override_settings(
        NL2SQL_EMBEDDING_PROVIDER="LM_STUDIO",
        NL2SQL_OLLAMA_EMBEDDING_MODEL="snowflake-arctic-embed2",
        NL2SQL_OLLAMA_BASE_URL="http://example-ollama:11434",
    )
    @patch("webapps.vanna.embedding_factory._build_ollama_embeddings")
    ...
    @override_settings(
        NL2SQL_EMBEDDING_PROVIDER="OPENAI",
        NL2SQL_OPENAI_EMBEDDING_MODEL="text-embedding-3-small",
    )
    @patch("webapps.vanna.embedding_factory._build_openai_embeddings")
    ...
    @override_settings(NL2SQL_EMBEDDING_DIMENSION=1024)

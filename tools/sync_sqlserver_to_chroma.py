...
embdding_function:OllamaEmbeddings = None

try:
    from dotenv import load_dotenv  # type: ignore
    load_dotenv()
    embdding_modelname = "nomic-embed-text"
    embdding_function = OllamaEmbeddings(model=embdding_modelname)
except Exception:
    pass
...
def langchain_embed(texts: List[str]) -> List[List[float]]:
    pass
...
def ollama_embed(texts: List[str]) -> List[List[float]]:
    base_url = env("OLLAMA_BASE_URL", "http://mpcai.mpc.mil.tw:11434").rstrip("/")
...
class ChromaIndex:
    ...
    def upsert(self, ids: List[str], texts: List[str], metas: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            print(f"[SUCCESS] 成功写入 {len(ids)} 个文件")
        except Exception as e:
...
def get_sqlserver_conn() -> pyodbc.Connection:
    try:
        ...
        db = os.getenv('SQL_SERVER_DATABASE', 'CaseManager')

        print(f"[CONN] Using host={host}, database={db}")

        conn_str = (
            f"DRIVER={{ODBC Driver 17 for SQL Server}};"
            f"SERVER={host}\\mpcsqlserver;"
            f"DATABASE={db};"
            f"UID={os.getenv('SQL_SERVER_USER', 'sa')};"
            f"PWD={os.getenv('SQL_SERVER_PASSWORD', 'Aa123456')};"
        )

        print(f"[CONN] Connection string: {conn_str}")
        ...
        while True:
            ...
            for d in dicts:
                ...
                if not case_id or not item_no:
                    print(f"[SKIP] 缺少关键字段，跳过记录: CaseID={case_id}, ItemNo={item_no}")
                    continue
